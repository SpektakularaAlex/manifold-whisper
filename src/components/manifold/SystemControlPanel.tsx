...
export function FamilyPicker({
  ...
  function getSubItems(
    fam: FamilyMeta,
  ): Array<{ libr: number | null; branch: string | null; label: string }> {
    ...
    if (fam.requiresBranch) {
      return fam.availableBranches.map((b) => ({
        libr: null,
        branch: b,
        label: b === "N" ? "North" : "South",
      }));
    }
    return [];
  }
            ...
            return (
              <div key={fam.id}>
                <div
                  ...
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.background = famSelected
                      ? "rgba(0,255,255,0.15)"
                      : "transparent";
                  }}
                  style={{
                ...
                {!isLeaf &&
                  expanded &&
                  getSubItems(fam).map((sub) => {
                    const sel =
                      selectedFamily?.id === fam.id &&
                      selectedLibr === sub.libr &&
                      selectedBranch === sub.branch;
                    return (
                      <div
                        key={`${sub.libr}-${sub.branch}`}
                        onClick={() => {
                          onSelect(fam, sub.libr, sub.branch);
                          setIsOpen(false);
                        }}
                        onMouseEnter={(e) => {
...
export function SystemControlPanel({
  ...
  const [showManifolds, setShowManifolds] = useState<"none" | "unstable" | "stable" | "both">(
    "none",
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        width: isExpanded ? 720 : 190,
        maxWidth: "calc(100vw - 48px)",
        zIndex: 10,
        padding: isExpanded ? "12px 16px" : "9px 11px",
    ...
    >
      <button
        ...
        style={{
          ...
          padding: isExpanded ? "5px 7px" : "7px 9px",
        ...
        <>
          ...
          {lastOrbitMeta && (
            ...
            >
              ...
              {lastOrbitMeta.totalMembers != null &&
                `  ·  Family size: ${lastOrbitMeta.totalMembers} members`}
            </div>
          )}
        </>
      )}
